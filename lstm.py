...
class RNN(nn.Module):
    def __init__(self, seq_len, embedding_dim=300, hidden_size=100, num_layers=2):
        ...
        self.rnn = nn.LSTM(embedding_dim, hidden_size, num_layers)  # 输入维度embedding_dim，输出维度hidden_size，其中全连接层数num_layers
    ...
    def forward(self, x):
        ...
        _, B, E = x.shape
        c0 = torch.randn(self.num_layers, B, self.hidden_size)  # c0的shape默认和h0一致
